...
class ExtractCommand(Command):
    ...
    summary = 'Symbolic link packages'
    ...
    def run(self, options, args):
        ...
        if not args:
            raise InstallationError(
                'You must give at least one package to link from')
        packages = []
        for arg in args:
        ...
        for module_name, filename in packages:
            last_status = self.link_package(module_name, filename, options.no_pyc)
        return last_status

    def link_package(self, module_name, filename, no_pyc):
        orig_filename = filename
        logger.notify('Link %s (in %s)' % (module_name, display_path(filename)))
        logger.indent += 2
        ...
        try:
            ...
            if not self.simulate:
                if not os.path.exists(dest_filename):
                    os.symlink(orig_filename, dest_filename)
                else:
                    logger.notify('The destination path:%s is not empty' % dest_filename)
        except:
